...
displayTradingDays = 5  # from task B.3 select n trading days for chart to display.
...
def load_process_data(TRAIN_START, TRAIN_END, date_split=True, split_ratio=0.3, store_data=True,
    ...
    if os.path.isfile("datafile"):
        ...
        # Generates a pandas data structure that contains dates associated with the elements listed in feature columns.
    ...
    if date_split:
        ...
        # this line takes in the split ratio and finds its opposite value for train samples,
        # it then multiplies by the amount of prediction days.
        ...
        # Uses train_test_split import which uses given ratio parameter
        # and two numpy arrays to split another array randomly between them.
    x_train = X
    y_train = y
    ...
    # input sample, then there are potentially 2 issues: 1. If you're training
    ...
    load_test_data(TRAIN_START, TRAIN_END, COMPANY, data, model, scaler, PREDICTION_DAYS, displayType)
...
def displayBoxPlot(data, COMPANY):
    chart_small = data[-displayTradingDays:]  # set number of days to display
    plt.bar(x=chart_small["Date"], height=chart_small["Close"], color="Blue")
    # create bar chart with x-axis as dates, y is daily closing price.

    # plt.tight_layout possibly deprecated
    plt.xlabel("Date")
    plt.ylabel(f"{COMPANY} Share Price")
    plt.title("Bar Chart")
    plt.subplots_adjust(bottom=0.25)  # Make sure dates don't get cutoff screen
...
load_process_data('2022-01-01', '2023-01-01', True, 0.3, True, 0, 1)
