...
def load_process_data(TRAIN_START, TRAIN_END, date_split=True, split_ratio=0.3, store_data=True,
    ...
    model = build_model(x_train, y_train, LSTM_Units, Dense_Units, Dropout_Amt, Epoch_Count, Batch_Count)
    load_test_data(TRAIN_START, TRAIN_END, COMPANY, data, model, scaler, PREDICTION_DAYS,
                   displayType)


def build_model(x_train, y_train, unitsLSTM, unitsDense, dropOutAmt, epochAmt, batchAmt):
