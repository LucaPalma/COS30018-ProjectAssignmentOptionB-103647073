...
# ------------------------------------------------------------------------------
# Load the test data and make a prediction
def load_test_data(TRAIN_START, TRAIN_END, COMPANY, data, model, scaler, PREDICTION_DAYS, chart_type):
    ...
    # Charts data and Makes final predicted price.
    makePrediction(chart_type, actual_prices, predicted_prices, data, model_inputs, model, scaler)


def makePrediction(chart_type, actual_prices, predicted_prices, data, model_inputs, model, scaler):
    if chart_type == "graph":
